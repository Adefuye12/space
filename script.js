...
const destinationTitle = document.querySelector('.destination-info h3');
const destinationDescription = document.querySelector('.destination-info p');
const destinationDistance = document.querySelector('.destination-stats div:nth-child(1) p');
const destinationTravel = document.querySelector('.destination-stats div:nth-child(2) p');

destinationTabs.forEach(tab => {
    tab.addEventListener('click', () => {
        const destinationName = tab.getAttribute('data-destination');
        const destination = data.destinations.find(d => d.name === destinationName);
...
// Technology Tabs
const techTabs = document.querySelectorAll('.tech-tabs button');
const techImage = document.querySelector('.tech-image img');
const techTitle = document.querySelector('.tech-info h3');
const techDescription = document.querySelector('.tech-info p');

techTabs.forEach(tab => {
    tab.addEventListener('click', () => {
        const techName = tab.getAttribute('data-tech');
        const technology = data.technology.find(t => t.name === techName);

        techTabs.forEach(t => t.classList.remove('active'));
        tab.classList.add('active');

        techImage.src = window.innerWidth > 768 ? technology.images.portrait : technology.images.landscape;
...
// Responsive image for technology
window.addEventListener('resize', () => {
    const activeTechTab = document.querySelector('.tech-tabs button.active');
    if (activeTechTab) {
        const techName = activeTechTab.getAttribute('data-tech');
        const technology = data.technology.find(t => t.name === techName);
        techImage.src = window.innerWidth > 768 ? technology.images.portrait : technology.images.landscape;
    }
});